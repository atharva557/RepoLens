...
| `test_notify.py` | PR-review notification: the report renders to text + HTML with warnings/oks/summary, singular-vs-plural subject, a bare report (no LLM summary, no url) still renders, **report text is HTML-escaped** (warnings carry file paths and LLM output), `PR_REVIEW_EMAIL` is opt-in and silent with no recipients, recipients are the repo's trackers plus a deduped `NOTIFY_EMAIL`, and **one refused address or a dead identity lookup never fails the review**. `ConsoleMailer` + `MemoryIdentity`; no network. |
