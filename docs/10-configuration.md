...
**Finished PR reviews are emailed too**, using this same account — there is no
separate setting to turn on. In multiuser mode they go to everyone tracking
that repo (`user_repos`), which is exactly the set already permitted to read
the report, so a notification can never surface a repo the recipient couldn't
open anyway. With no accounts (or a repo nobody tracks) they fall back to
`SMTP_USER`, so a single-user setup gets its own reviews with no extra config.
Both the webhook and `POST /repos/{key}/pr-reviews/{n}` notify; the report is
persisted either way, and a mail failure never fails the review. With SMTP
unset the console backend prints them, exactly as it does verification codes.
